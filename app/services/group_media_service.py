...
def get_all_group_media(
    ...
    group_media: list[GroupMedia] = list(db.scalars(select(GroupMedia).where(GroupMedia.group_id == group_id)))

    return [GroupMediaRead.model_validate(m) for m in group_media]


def create_group_media(
    db: Session,
    ...
    user_group = db.scalar(
        ...
        .where(
            ...
            UserGroup.user_id == payload.uploaded_by))
