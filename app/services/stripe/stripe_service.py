...
from app.services.stripe.stripe_next_card_service import (
    get_next_card_number,
    update_next_card_number,
)
...
class StripeService:
    ...
    def deposit_funds(
        ...
        amount: Decimal,
        currency: Currency,
        description: str = "Balance deposit",
    ):
        payment_intent = stripe.PaymentIntent.create(
            ...
            metadata={"type": "deposit", "description": description},
        )
    ...
    def get_customer_balance(customer_id: str):
        payment_intents = stripe.PaymentIntent.list(customer=customer_id, limit=100)
...
class StripeTestCards:
    ...
    async def get_next_card(db: AsyncSession):
        ...
        if number > 11:
            raise HTTPException(
                status_code=400, detail="We are run out of test cards, sorry :)"
            )
